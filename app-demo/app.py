...
class MainApplication:
												...
												children=[
													...
													dmc.TabsPanel(
														...
														children=[
															dcc.Loading(
																type="dot",
																target_components={ "time-series-section": "children" },
																children=[
																	dmc.Grid(
	...
	def _register_callbacks(self):
		...
		def update_orders_insights_section(date_from, date_to):
			try:
				loop = asyncio.new_event_loop()
				asyncio.set_event_loop(loop)
				results = loop.run_until_complete(
					self.query_manager.execute_queries(
						query_names=[
							"order_sales_stats",
		...
		@self.__app.callback(
			output=(dash.Output("time-series-section", "children")),
			inputs=(dash.Input("filter-date-from", "value"), dash.Input("filter-date-to", "value")),
			prevent_initial_call=False,
			background=True,
			manager=background_callback_manager
		)
		def update_time_series_section(date_from, date_to):
			try:
				loop = asyncio.new_event_loop()
				asyncio.set_event_loop(loop)
				results = loop.run_until_complete(
							...
							dmc.Card(
								...
								children=[
											...
											dmc.Group(
												children=[
													dmc.SegmentedControl(
														...
														value="value",
														data=[
															{ "value": "value", "label": "Show Value" },
															{ "value": "volume", "label": "Show Volume" }
														],
													),
													dmc.Switch(
									...
									dmc.LineChart(
										...
										withRightYAxis=True,
										yAxisLabel="regular_sales_amount",
										rightYAxisLabel="transaction_count",
										series=[
											{ "name": "regular_sales_amount", "label": "Sales Value", "color": "blue" },
											{ "name": "transaction_count", "label": "Transactions", "color": "gray", "yAxisId": "right" }
										],
										referenceLines=[
											*[{
												"x": row['start_time'],
												"label": row['entry_name'],
												"color": "gray" if row['stage_name'] != '-' else "red",
											} for _, row in event_entry_timeline.iterrows()]
										],
										yAxisProps={ "width": 80 },
										withLegend=True,
									),
		...
		def update_cash_flow_section(date_from, date_to):
			try:
				loop = asyncio.new_event_loop()
				asyncio.set_event_loop(loop)
				results = loop.run_until_complete(
