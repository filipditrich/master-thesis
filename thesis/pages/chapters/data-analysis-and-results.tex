...
\begin{figure}[H]
	\centering
	\small
	\begin{tabularx}{0.95\textwidth}{|>{\columncolor{unicorn_blue!5}}X|>{\columncolor{unicorn_blue!5}}r|>{\columncolor{unicorn_blue!5}}r|}
		\hline
		\rowcolor{unicorn_blue}
		\textbf{\color{white}Payment Method}   & \textbf{\color{white}Count} & \textbf{\color{white}Total Value (CZK)} \\
		\hline
		\hline
		\colorindicator{chart1}Card terminal     & \fmtnum{8486}               & \fmtczk{7264503}                        \\
		\colorindicator{chart2}Cash              & \fmtnum{7561}               & \fmtczk{5782570}                        \\
		\colorindicator{chart3}Online pre top-up & \fmtnum{1634}               & \fmtczk{1436400}                        \\
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{\ResultsDir/rq4-organizer-vs-vendor-sales}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{\ResultsDir/rq25-beverage-preferences}
...
Key observations:
\begin{itemize}
	\item Non-alcoholic beverages dominate sales in the morning hours (6:00-11:00).
	\item Alcoholic beverage sales surge in the afternoon, peaking between 17:00-19:00.
	\item The proportion of alcoholic sales remains high into the late evening hours.
	\item Non-alcoholic sales see a smaller resurgence in the late evening, likely due to customers alternating between alcoholic and non-alcoholic drinks.
\end{itemize}
...
Identifying frequently purchased product combinations can reveal important insights about customer preferences and behaviors.
Two analyses were conducted: one including returnable cups and one excluding them, as the high frequency of cup purchases can obscure other meaningful patterns.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{
		...
		\hline
		\rowcolor{unicorn_blue}
		\textbf{\color{white}Product 1} & \textbf{\color{white}Product 2} & \textbf{\color{white}Combo Count}
		& \textbf{\color{white}\% of Transactions}
		\\
		\hline
		\csvreader[head to column names, late after line= \\]{rq26productcomboswithreturnable.csv}
		{product1=\product, product2=\product, combination_count=\combos, percentage_of_all_transactions=\percentage}
	...
	\label{tab:common-combos-with-cups}
\end{table}

As expected, returnable cups feature prominently in the most common combinations.
To reveal additional insights, the analysis was repeated excluding cup transactions.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{
		...
		\\
		\hline
		\csvreader[head to column names, late after line= \\]{rq26productcombosnonreturnable.csv}
		{product1=\product, product2=\product, combination_count=\combos, percentage_of_all_transactions=\percentage}
		{\product & \product & \num[group-separator={,}]{\combos} & \num[round-precision=2]{\percentage}\% }
		\hline
	\end{tabularx}
	\caption{Most Common Product Combinations (Excluding Returnable Cups)}
	\label{tab:common-combos-no-cups}
\end{table}

Key takeaways:
\begin{itemize}
	\item Popular beers like Radegast and Pilsner Urquell are frequently purchased together, suggesting customers often buy rounds of the same beer.
	\item Radegast is also commonly bought with non-alcoholic Birell, possibly indicating designated drivers or customers pacing their alcohol consumption.
	\item Cider (Kingswood) and select spirits (Beefeater, Jameson) make notable appearances, revealing their popularity as complements or alternatives to beer.
\end{itemize}

This analysis provides a data-driven foundation for optimizing product placement, bundling, and cross-promotion strategies to drive incremental sales and enhance the customer experience.