...
\begin{section}{Cashflow and Revenue Sources Analysis}
	...
	\begin{tabularx}{0.95\textwidth}{
		...
		\hline
		\colorindicator{3}Online Top-Up Voucher & \fmtnum{1634}& \fmtczk{1436400}   \\
		\hline
	...
	\begin{subsection}{Total Revenue of the Organizer}
		...
		To better understand the revenue streams, the results are visualized in~\autoref{chart:revenue-breakdown-total}.
	...
	\begin{tabularx}{\textwidth}{
		|>{\columncolor{unicorn_blue!5}\centering\arraybackslash}p{0.5cm}
...
\begin{section}{Performance Indicators Analysis}
	...
	\begin{subsection}{Transactions Processing}
		...
		It clearly identifies the peak on the last day of the festival at 18:00, totaling to~\bfmtnum{8986}~transactions.
	...
	%%% Performance / Best Places, Vendors, and Products Analysis
	%%% --------------------------------------------------------------
	\begin{subsection}{Best Places, Vendors, and Products}
		...
		%%% Performance / Best Places, Vendors, and Products Analysis / Best Sale Places
		%%% --------------------------------------------------------------
	...
	\begin{tabularx}{\textwidth}{
		...
		|>{\columncolor{unicorn_blue!5}\raggedright\arraybackslash}X
		|>{\columncolor{unicorn_blue!5}\raggedleft\arraybackslash}p{2cm}
		|>{\columncolor{unicorn_blue!5}\raggedleft\arraybackslash}p{2.5cm}
		|>{\columncolor{unicorn_blue!5}\raggedleft\arraybackslash}p{2.75cm}|}
		\hline
		\rowcolor{unicorn_blue}
		\textbf{}
...
%		filter={\thecsvinputline<7}
	...
	% @formatter:on
		...
		\begin{subsubsection}{Best Sale Places}
			...
			Out of the total of~\bfmtnum{134}~sale places, shown in \autoref{tab:best-sale-points} below,
			the best place was undeniably the \textbf{L20 PIVNÍ STAN 1} with a total of~\bfmtnum{10114}~orders processed and the maximum peak of~\bfmtnum{840}~orders per hour.
	...
	\begin{tabularx}{\textwidth}{
		...
		|>{\columncolor{unicorn_blue!5}\raggedleft\arraybackslash}p{2cm}
		|>{\columncolor{unicorn_blue!5}\raggedleft\arraybackslash}p{2cm}
		|>{\columncolor{unicorn_blue!5}\raggedleft\arraybackslash}p{3cm}|}
		\hline
		\rowcolor{unicorn_blue}
		\textbf{}
	...
	% @formatter:on
		...
		%%% Performance / Best Places, Vendors, and Products Analysis / Best Vendors
		%%% --------------------------------------------------------------
		\begin{subsubsection}{Best Vendors}
			...
			The results in~\autoref{tab:best-vendors} show the distribution of the processed orders over the \bfmtnum{27}~vendors at the festival.
	...
	\begin{tabularx}{\textwidth}{
		...
		% separator
		\noalign{\vspace{1mm}}
		\multicolumn{4}{c}{\footnotesize{\textellipsis}}
		\\
		\noalign{\vspace{1mm}}
		\hline
		% last rows
	...
	% @formatter:on
			...
			\begin{keytakeaways}
				\begin{itemize}
					...
					\item They processed over~\bfmtnum{9800}~unique customers (\bfmtnump[2]{98.22}~\%~of all active customers).
		...
		%%% Performance / Best Places, Vendors, and Products Analysis / Best Products
		%%% --------------------------------------------------------------
	...
	% @formatter:on
			...
			This can be seen in~\autoref{chart:best-product-category}.
...
\begin{section}{Beverage Consumption Analysis}
	...
	\begin{subsection}{Returnable Cups}
		...
		Upon calculating the total number of issued and returned cups, the results depicted in~\autoref{chart:returnable-cups} illustrate the distribution of returnable cups throughout the event.
	...
	\begin{subsection}{Total Consumption}
		...
		These results are shown in~\autoref{tab:beverage-total-consumption} below and show a total of~\bfmtnump[2]{35843.79}~liters of beverages consumed during the event, with the beer category being the most consumed.
	...
	\begin{tabularx}{\textwidth}{
		|>{\columncolor{unicorn_blue!5}\centering\arraybackslash}p{0.5cm}
		|>{\columncolor{unicorn_blue!5}\raggedright\arraybackslash}X
		|>{\columncolor{unicorn_blue!5}\raggedright\arraybackslash}r
		|>{\columncolor{unicorn_blue!5}\raggedright\arraybackslash}r|
	r}
		\hline
		\rowcolor{unicorn_blue}
		...
		\hline
		\hline
		\colorindicator{1} & Beer & \fmtnump[2]{25883.3}~l & \fmtnump[2]{72.21}~\% \\\hline
		\colorindicator{2} & Non-alcoholic & \fmtnump[2]{7832.47}~l & \fmtnump[2]{21.85}~\% \\\hline
		\colorindicator{3} & Spirits / other alcohol & \fmtnump[2]{1255.44}~l & \fmtnump[2]{3.50}~\% \\\hline
		\colorindicator{4} & Wine & \fmtnum{872.58}~l & \fmtnump[2]{2.44}~\% \\\hline
		\noalign{\vspace{1mm}}
		\hline
		\rowcolor{unicorn_blue!20}
		\textbf{} & \textbf{Total} & \bfmtnump[2]{35843.79}~l & \fmtnum{100}~\% \\\hline
	\end{tabularx}
	% @formatter:on
...
% rows
			...
			\footnote{The price of Radegast ranged from~\fmtczk{60} to~\fmtczk{66} and Pilsner Urquell was priced at~\fmtczk{60} to~\fmtczk{75}, depending on the beer size and its grade.}.
...
\begin{section}{Customer Analysis}
	...
	\begin{subsection}{Event Attendance and Timeline}
		...
		\begin{subsubsection}{Visitor Arrival Patterns}
			...
			\begin{keytakeaways}
				\begin{itemize}
					...
					\item The weekend saw renewed activity with the peak of~\bfmtnum{728}~new visitors.
				\end{itemize}
			\end{keytakeaways}
		\end{subsubsection}
		...
		\begin{subsubsection}{Time to First Transaction}
			...
			The analysis results reveal significant variations in how quickly different types of visitors made their first transaction after arrival.
	...
	\begin{subsection}{Purchase Patterns}
		...
		\begin{subsubsection}{Common Product Combinations}
			...
			The initial results are shown in~\autoref{chart:common-combos-with-cups} and as expected, returnable cups appear prominently in the most popular combinations as it was necessary to purchase them for drinks.
	...
	% @formatter:on
			...
			To gain additional insights, the analysis was repeated while excluding returnable cups, as shown in~\autoref{chart:common-combos-without-cups}.
	...
	% @formatter:on
			...
			This revealed that the most popular combination, without returnable cups, was \textbf{Absolut Vodka} and \textbf{Redbull} found in total of~\bfmtnump[2]{2.48}~\%~of all transactions.
			...
			\begin{keytakeaways}
				\begin{itemize}
					...
					\item Cups were a significant part of the most common combinations.
				\end{itemize}
			\end{keytakeaways}
		\end{subsubsection}
	\end{subsection}
