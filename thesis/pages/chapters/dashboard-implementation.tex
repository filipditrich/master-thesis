...
\begin{section}{Development Approach}
	...
	\begin{subsection}{Technology Selection}
		...
		The dashboard was built using Dash and Plotly, chosen primarily for their familiarity from prior personal experience in the academic field and their great capability for relatively fast data app prototyping\cite{plotly_dash_plotly_com}.
...
query_manager.registry.register_query(
	...
	\begin{subsection}{Dual Caching Strategy}
		...
		\begin{subsubsection}{Query Result Caching}
			...
			Thus, a basic file-based caching system was implemented to store query results as CSV files, preventing redundant database queries during development.
...
background_callback_manager = dash.DiskcacheManager(
    ...
    expire=300
		...
		\begin{subsubsection}{First Solution Attempt}
			...
			To address cache key collisions, we tried to implement a unique~\textt{DiskacheManager} instance for each unique callback, based on it's~\texttt{callback\_id}:
